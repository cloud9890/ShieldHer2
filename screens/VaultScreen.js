...
import { draftComplaint } from "../services/claude";
      ...
      <View style={s.captureCard}>
        ...
        <View style={s.captureRow}>
          {[
            ...
            { icon: "location",       label: "GPS",      color: "#06b6d4" },
            { icon: "document-text",  label: "Describe", color: "#f59e0b" },
          ].map(b => (
            <TouchableOpacity key={b.label} style={[s.captureBtn, { borderColor: b.color + "40" }]}
              onPress={() => { if (b.label === "GPS") captureLocation(); else setShowForm(true); }}>
