...
export default function ProfileScreen() {
  ...
  const saveSetting = async (key, val) => {
    try {
      ...
      showToast(
        "Setting could not be saved. Check your connection.",
        "warning",
      );
  ...
  const processImageResult = async (result) => {
    ...
    try {
      ...
      await supabase
        .from("profiles")
        .upsert({ id: user.id, avatar_url: pubData.publicUrl });
      ...
      Alert.alert(
        "Upload Failed",
        err.message ||
          "Could not upload photo. Make sure the 'avatars' bucket exists in Supabase Storage.",
      );
    }
    setUploading(false);
  };
  ...
  const Card = ({ title, children, redBorder }) => (
    <View style={s.section}>
      ...
      <View
        style={[
          s.sectionCard,
          redBorder && { borderColor: "rgba(239,68,68,0.25)" },
        ]}
      >
        {children}
      </View>
          ...
          <View style={s.editForm}>
            ...
            >
              {saving ? (
                ...
                <>
                  <CloudCheck size={16} color="white" />
        ...
        <Row
          ...
          right={
            <Switch
              ...
              onValueChange={(v) => {
                setShakeOn(v);
                saveSetting("shake_on", v);
              }}
              trackColor={{ true: PRIMARY + "80" }}
              thumbColor={shakeOn ? PRIMARY : "#6b7280"}
            />
          }
        />
        <Row
          ...
          right={
            <Switch
              ...
              onValueChange={(v) => {
                setNotifOn(v);
                saveSetting("notif_on", v);
              }}
              trackColor={{ true: PRIMARY + "80" }}
              thumbColor={notifOn ? PRIMARY : "#6b7280"}
            />
          }
        />
        <Row
          ...
          right={
            <Switch
              ...
              onValueChange={(v) => {
                setGuardOn(v);
                saveSetting("guard_on", v);
              }}
              trackColor={{ true: "#34d39980" }}
              thumbColor={guardOn ? "#34d399" : "#6b7280"}
            />
          }
        />
        <Row
          ...
          sub="Require fingerprint to open Vault"
          last
          right={
            <Switch
              value={biometricOn}
              onValueChange={(v) => {
                setBiometricOn(v);
                saveSetting("biometric_on", v);
              }}
              trackColor={{ true: PRIMARY + "80" }}
              thumbColor={biometricOn ? PRIMARY : "#6b7280"}
            />
          }
        />
      </Card>
