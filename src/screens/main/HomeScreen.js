...
import {
  ...
  setGlobalShakeCallback,
  startAutoDangerDetection,
...
export default function HomeScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const timeOfDay =
          h < 6 ? "late night" : h < 18 ? "daytime" : "evening/night";
        const alertsText =
          communityAlerts.length > 0
            ? `${communityAlerts.length} community reports nearby.`
            : "No immediate community reports.";
      ...
      : [
          {
            name: "Emergency",
            phone:
              process.env.EXPO_PUBLIC_TWILIO_VERIFIED_NUMBER || "+918310661631",
          },
        ];
  ...
  const activateSOS = async () => {
    ...
    startEvidenceRecording().catch((e) => {
      ...
      Alert.alert(
        "Evidence Fault",
        "Failed to boot microphone buffer. Please verify security permissions.",
      );
    });
  };
    ...
    try {
      ...
      if (recordingUri) {
        ...
        if (user) {
          ...
          Alert.alert(
            "📁 Recording Saved",
            "The SOS audio recording has been saved to your Evidence Vault.",
          );
        }
      }
    } catch (e) {
  ...
  const toggleGuardian = async () => {
    ...
    try {
      ...
      if (user) {
        await supabase
          .from("profiles")
          .upsert({ id: user.id, guard_on: next, updated_at: new Date() });
      }
    } catch (_) {}
  };
    ...
    return (
      <View style={s.sosBg}>
        ...
        <View style={s.sosActiveCard}>
          {[
            "Live location sent to contacts",
      ...
      <View style={s.dangerBg}>
        <View style={s.dangerCard}>
          ...
          <Text style={s.dangerBody}>
            {DANGER_REASONS[dangerAlert.reason] || "Possible danger detected."}
          </Text>
  ...
  return (
    <View style={s.container}>
        ...
        <View style={s.headerInner}>
          <View style={{ flex: 1 }}>
            ...
            <Text style={s.headerTitle}>
              Stay Safe, {profile.name.split(" ")[0]} 💜
            </Text>
          </View>
          <Hoverable
          ...
          >
            {imageUri ? (
              ...
              >
                <Text style={[s.thumbText, { color: avatarColor }]}>
                  {initials}
                </Text>
              </View>
            )}
      ...
      <Animated.View
        style={{ opacity: fadeAnim1, transform: [{ translateY: slideAnim1 }] }}
      >
        {/* Guardian Mode */}
        <Pressable
        ...
        >
          <GuardianBadge guardianOn={guardianOn} pulseAnim={pulseAnim} />
        </Pressable>
        ...
        <View style={s.sosSection}>
          <Animated.View
            style={[
              s.ring,
              s.ring3,
              { transform: [{ scale: ring3 }], opacity: ringOpacity3 },
            ]}
          />
          <Animated.View
            style={[
              s.ring,
              s.ring2,
              { transform: [{ scale: ring2 }], opacity: ringOpacity2 },
            ]}
          />
          <Animated.View
            style={[
              s.ring,
              s.ring1,
              { transform: [{ scale: ring1 }], opacity: ringOpacity1 },
            ]}
          />
            ...
            <View style={s.advisorHeader}>
              ...
              <View
                style={[
                  ...
                  {
                    backgroundColor:
                      advisorBrief.riskLevel === "high"
                        ? "rgba(239,68,68,0.25)"
                        : advisorBrief.riskLevel === "moderate"
      ...
      {/* Community Alerts */}
      <Animated.View
        style={{ opacity: fadeAnim3, transform: [{ translateY: slideAnim3 }] }}
      >
        ...
        {communityAlerts.length === 0 ? (
          <View style={[s.alertCard, s.shadowCard]}>
            ...
            <Text style={[s.alertText, { color: SUBTEXT }]}>
              No reports nearby — all clear!
            </Text>
          </View>
        ) : (
          communityAlerts.map((a, i) => {
            const cat =
              REPORT_CATEGORIES[a.category] || REPORT_CATEGORIES["Other"];
            return (
              <Pressable
                key={a.id || i}
                onPress={() => {}}
...
const s = StyleSheet.create({
  ...
  header: { paddingBottom: 20, paddingHorizontal: 20, backgroundColor: "transparent" },
  headerInner: {
    flexDirection: "row",
  ...
  dangerBg: {
    flex: 1,
    backgroundColor: "rgba(13,11,23,0.97)",
    alignItems: "center",
    justifyContent: "center",
    padding: 24,
  },
