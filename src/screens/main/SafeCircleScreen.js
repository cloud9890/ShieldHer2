...
export default function SafeCircleScreen() {
  ...
  const {
    ...
    removeContact: removeContactFromSupabase,
  } = useContacts();
  ...
  const triggerSOS = async () => {
    if (contacts.length === 0) {
      Alert.alert(
        "No Contacts",
        "Please add contacts to your SafeCircle first.",
      );
      return;
    }
    Alert.alert(
      "🚨 Trigger Emergency SOS?",
      "This will send your live location to all contacts immediately.",
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            ...
            if (customContext.trim()) {
              try {
                customMsg = await personaliseSOSMessage(
                  customContext,
            ...
            if (res.success)
              Alert.alert("✅ SOS Dispatched", `Sent to ${res.sent} contacts.`);
  ...
  const startEscort = async () => {
    ...
    if (status !== "granted") {
      Alert.alert(
        "Permission Required",
        "Location access is required for live escort.",
      );
      return;
    }
    try {
      ...
      if (customContext.trim()) {
        try {
          customMsg = await personaliseSOSMessage(
            customContext,
            `${liveLocRef.current?.latitude}, ${liveLocRef.current?.longitude}`,
          );
        } catch (e) {}
      }
  ...
  const endEscort = async () => {
    ...
    if (res.success) {
      Alert.alert(
        "✅ Safe Check-in Sent",
        `${res.sent} contact${res.sent !== 1 ? "s" : ""} notified that you arrived safely.`,
      );
    }
  };
  ...
  const removeContact = (id) =>
    Alert.alert("Remove Contact", "Remove from your SafeCircle?", [
      { text: "Cancel", style: "cancel" },
      {
        text: "Remove",
        style: "destructive",
        onPress: () => removeContactFromSupabase(id),
      },
    ]);
    ...
    <ScrollView style={s.container} showsVerticalScrollIndicator={false}>
      ...
      <Hoverable style={s.sosHeroBtn} onPress={triggerSOS}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={s.sosHeroSub}>
            Send location to all contacts instantly
          </Text>
        </View>
      ...
      <View style={{ marginHorizontal: 16, marginBottom: 16 }}>
        ...
        <Text style={s.aiContextHint}>
          If provided, AI will generate a custom distress SMS using this
          context.
        </Text>
      </View>
      ...
      <Animated.View
        style={[
          s.escortCard,
          escortActive && s.escortCardActive,
          escortActive && { transform: [{ scale: pulseAnim }] },
        ]}
      >
        <View style={s.escortHeader}>
          <Navigation size={20} color={escortActive ? SUCCESS : PRIMARY} />
          <Text style={[s.escortTitle, escortActive && { color: SUCCESS }]}>
            Live Escort
          </Text>
        ...
        {escortActive ? (
          <>
            <Text style={s.escortSub}>
              {trackUrl
                ? "Contacts receiving live tracking link 🔴"
            ...
            {trackUrl && (
              <View style={s.trackUrlBox}>
                ...
                <Text style={s.trackUrlText} numberOfLines={1}>
                  {trackUrl}
                </Text>
              </View>
            )}
          ...
          <>
            <Text style={s.escortSub}>
              Contacts get a real-time tracking link updating every second.
            </Text>
            <Hoverable
      ...
      <View style={s.card}>
        <View style={s.cardHeader}>
          ...
          <Hoverable
            style={[s.addBtn, { borderColor: "rgba(239,68,68,0.4)" }]}
                ...
                onPress={() =>
                  Alert.alert("Delete Alert", "Remove report?", [
                    { text: "Cancel", style: "cancel" },
                    {
                      ...
                      style: "destructive",
                      onPress: () =>
                        saveReports(reports.filter((x) => x.id !== r.id)),
                    },
                  ])
                }
              >
          ...
          <View style={s.modalCard}>
            ...
            <View style={s.catGrid}>
              {REPORT_CATEGORIES.map((c) => (
                ...
                >
                  <c.Icon
                    size={14}
                    color={selectedCat === c.label ? "white" : SUBTEXT}
                  />
                  <Text
                    style={[
                      s.catBtnText,
                      selectedCat === c.label && { color: "white" },
                    ]}
                  >
                    {c.label}
                  </Text>
            ...
            <View style={s.modalInputRow}>
              ...
              <TextInput
                style={s.modalInput}
                placeholderTextColor={MUTED}
                placeholder="Area name (blank = auto-pin)"
                value={reportArea}
            ...
            <View style={s.modalBtns}>
              ...
              <Hoverable
                style={[s.confirmBtn, { backgroundColor: DANGER }]}
