...
export default function VaultScreen() {
  ...
  const saveIncident = async (incidentData) => {
    try {
      ...
      showToast(
        "Record could not be saved: " + (e.message || "unknown error"),
        "error",
      );
      return null;
    }
  ...
  const processEvidenceImage = async (uri) => {
    ...
    try {
      ...
      setForm((prev) => ({
        ...
        location:
          analysis.location !== "Unknown" ? analysis.location : prev.location,
      }));
      Alert.alert(
        "✅ Analysis Complete",
        "AI has extracted details from your evidence. Review and generate a complaint.",
      );
    } catch (err) {
      console.error("AI Analysis:", err);
      Alert.alert(
        "Analysis Failed",
        "Could not scan evidence. Please enter details manually.",
      );
    } finally {
      setScanning(false);
    }
  ...
  const takePhoto = async () => {
    ...
    if (status !== "granted")
      return Alert.alert("Permission", "Camera access is needed.");
  ...
  const generate = async () => {
    if (!form.desc && !evidenceImage) {
      return Alert.alert(
        "Missing Info",
        "Please describe the incident or attach evidence.",
      );
    }
    setLoading(true);
    try {
        ...
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              ...
              showToast(
                "Could not delete record: " + (e.message || "unknown error"),
                "error",
              );
            }
          },
      ...
      >
        ...
        >
          ...
          <Hoverable
            ...
            onPress={() =>
              authenticate("Authenticate to access Evidence Vault")
            }
          >
      ...
      >
          ...
          <View style={s.headerRow}>
            <View>
              ...
              <Text style={s.subtitle}>
                {incidents.length} encrypted record
          ...
          <View style={s.captureRow}>
            {[
              ...
              >
                ...
                <Text style={[s.captureBtnText, { color: b.color }]}>
                  {b.label}
                </Text>
              </Hoverable>
            ))}
        ...
        {scanning && (
          <View style={s.scanCard}>
            ...
            <Text style={s.scanSub}>
              Extracting incident details with Gemini Vision
            </Text>
          </View>
        )}
        ...
        {showForm && !scanning && (
          <View style={s.formCard}>
            ...
            <Hoverable
              style={[s.draftBtn, loading && { opacity: 0.6 }]}
              onPress={generate}
              disabled={loading}
            >
              {loading ? (
            ...
            <Hoverable
              ...
              onPress={async () => {
                if (!Print || !Sharing) {
                  Alert.alert(
                    "Not Available",
                    "PDF export requires expo-print and expo-sharing.",
                  );
                  return;
                }
                try {
                  ...
                  if (await Sharing.isAvailableAsync()) {
                    await Sharing.shareAsync(uri, {
                      ...
                      dialogTitle: "Share Complaint PDF",
                    });
                  } else {
          ...
          {fetchingRecords ? (
            ...
            <View style={s.emptyState}>
              ...
              <Text style={s.emptySubText}>
                Your encrypted incident records will appear here
              </Text>
            </View>
          ) : (
            incidents.map((inc) => {
              ...
              return (
                ...
                >
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    {inc.location ? (
                      <Text style={s.recordLoc} numberOfLines={1}>
                        📍 {inc.location}
                      </Text>
                    ) : null}
                  </View>
