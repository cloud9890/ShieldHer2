...
import {
  ...
  EMERGENCY_NUMBERS,
  INCIDENT_TYPES,
} from "../../components/map/incidentMeta";
...
export default function NearbyScreen() {
        ...
        combined.slice(0, 10).map(async (item) => {
          ...
          try {
            const detailRes = await fetch(
              `https://maps.googleapis.com/maps/api/place/details/json?place_id=${item.place_id}&fields=formatted_phone_number&key=${GOOGLE_KEY}`,
            );
  ...
  const onReportTap = async (item) => {
    ...
    try {
      const aiReply = await summariseReport(
        item.category,
        item.note,
        new Date(item.created_at).toLocaleString(),
      );
      if (aiReply)
        setReportDetail({ ...item, aiSummary: aiReply, aiLoading: false });
  ...
  const onEscapeAdvice = async () => {
    ...
    try {
      ...
      const advice = await getEscapeAdvice(
        placesSubset,
  ...
  const filteredReports =
    filter === "all"
      ? reports
      : reports.filter((r) => getTypeMeta(r.category).key === filter);
      ...
      <View style={s.sosStrip}>
        {EMERGENCY_NUMBERS.map((e) => (
          <Hoverable
            ...
            style={[
              s.sosBtn,
              { borderColor: e.color + "40", backgroundColor: CARD },
            ]}
        ...
        >
          ...
          >
            {[1, 5, 10, 20].map((rad) => (
              <Hoverable
                key={rad}
                style={[s.chip, radiusKm === rad && s.chipActive]}
                onPress={() => setRadiusKm(rad)}
              >
                <Text
                  style={[s.chipText, radiusKm === rad && s.chipTextActive]}
                >
                  {rad} km
                </Text>
              </Hoverable>
          ...
          <ScrollView
            horizontal
            showsHorizontalScrollIndicator={false}
            style={[s.chipScroll, { marginTop: 12 }]}
            contentContainerStyle={{ gap: 8, paddingHorizontal: 16 }}
          >
            <Hoverable
            ...
            {INCIDENT_TYPES.map((f) => (
              ...
              >
                <Ionicons
                  name={f.icon}
                  size={14}
                  color={filter === f.key ? PRIMARY : SUBTEXT}
                />
                <Text
                ...
                <Hoverable key={p.id} onPress={() => setSelectedPlace(p)}>
                  <View
                    style={[
                      s.placeCard,
                      {
                        borderLeftColor: getTypeMeta(p.type).color,
                        borderLeftWidth: 3,
                      },
                    ]}
                  >
                    <View
                      style={[
                        s.placeIconWrap,
                        { backgroundColor: getTypeMeta(p.type).color + "18" },
                      ]}
                    >
                      <Ionicons
                        name={getTypeMeta(p.type).icon}
                        size={20}
                        color={getTypeMeta(p.type).color}
                      />
                    </View>
              ...
              <>
                ...
                >
                  ...
                  <Hoverable
                    ...
                    onPress={() =>
                      Linking.openURL(
                        `geo:0,0?q=${selectedPlace.lat},${selectedPlace.lng}`,
                      )
                    }
                  >
                    <Navigation size={18} color={PRIMARY} />
                    <Text style={[s.mBtnText, { color: PRIMARY }]}>
                      Navigate
                    </Text>
                  </Hoverable>
                </View>
                ...
                {!selectedPlace.advice ? (
                  ...
                  <View style={s.adviceBox}>
                    <View style={s.aiHeader}>
                      <CircleDashed size={14} color={WARNING} />
                      <Text style={s.aiTitle}>AI Escape Advice</Text>
                    </View>
                    {(selectedPlace.advice.steps || []).map((step, i) => (
                      <Text key={i} style={s.adviceStep}>
                        {i + 1}. {step}
                      </Text>
                    ))}
                  </View>
        ...
        <View style={s.modalOverlay}>
          <View
            style={[
              s.modalContent,
              {
                borderColor: reportDetail
                  ? getTypeMeta(reportDetail.category).color
                  : BORDER,
              },
            ]}
          >
              ...
              <>
                <View style={s.modalHeader}>
                  <Text style={s.modalTitle}>
                    {getTypeMeta(reportDetail.category).label}
                  </Text>
                  <Hoverable onPress={() => setReportDetail(null)}>
                    <X size={24} color={TEXT} />
                  </Hoverable>
                </View>
                <Text style={{ color: TEXT, fontSize: 15, marginBottom: 12 }}>
                  {reportDetail.note}
                </Text>
                <Text
                  style={{ color: SUBTEXT, fontSize: 12, marginBottom: 16 }}
                >
                  Reported: {new Date(reportDetail.created_at).toLocaleString()}
                </Text>
                ...
                <View style={s.adviceBox}>
                  <View style={s.aiHeader}>
                    <CircleDashed size={14} color={WARNING} />
                    <Text style={s.aiTitle}>AI Summary</Text>
                  </View>
                  {reportDetail.aiLoading ? (
                    ...
                    <>
                      ...
                      >
                        ...
                        <Text
                          style={{
                            color:
                              reportDetail.aiSummary.risk === "high"
                                ? DANGER
                                : WARNING,
                          }}
                        >
                          {(reportDetail.aiSummary.risk || "").toUpperCase()}
                        </Text>
                      </Text>
                      <Text style={s.adviceStep}>
                        {reportDetail.aiSummary.summary}
                      </Text>
                      <Text
                        style={[s.adviceStep, { marginTop: 6, color: WARNING }]}
                      >
                        Tip: {reportDetail.aiSummary.advice}
                      </Text>
                    </>
                  ) : (
