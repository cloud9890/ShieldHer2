...
export function IncidentCard({ item, onPress }) {
  ...
  return (
    <Hoverable
      style={[
        s.incCard,
        { borderColor: meta.color + "50", backgroundColor: meta.bg },
      ]}
      onPress={onPress}
    >
      <View style={s.incCardHeader}>
        <Ionicons name={meta.icon} size={14} color={meta.color} />
        <Text style={[s.incCardCategory, { color: meta.color }]}>
          {meta.label}
        </Text>
      </View>
...
export function PlaceCard({ place, onCall, onNavigate }) {
  ...
  return (
    <View
      style={[s.placeCard, { borderLeftColor: meta.color, borderLeftWidth: 3 }]}
    >
      ...
      <View style={s.placeInfo}>
        ...
        <Text style={s.placeAddr} numberOfLines={1}>
          {place.address || "Emergency service"}
        </Text>
        {place.rating ? (
          <View
            style={{
          ...
          >
            ...
            {place.open === true && (
              <Text style={[s.placeRating, { color: "#4ade80" }]}>· Open</Text>
            )}
            {place.open === false && (
              <Text style={[s.placeRating, { color: "#ef4444" }]}>
                · Closed
              </Text>
            )}
          </View>
      ...
      <View style={s.placeActions}>
        ...
        <Hoverable
          style={[
            s.actionBtn,
            { backgroundColor: PRIMARY + "18", borderColor: BORDER },
          ]}
