        ...
        <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 20 }}>
          <View style={s.modAvatarSection}>
            <Hoverable style={s.modAvatar} onPress={pickImage}>
              {imageUri ? (
                <Image source={{ uri: imageUri }} style={s.modImg} />
              ) : (
                <View
                  style={[
                    s.modInitials,
                ...
                >
                  <Text style={[s.modInitText, { color: avatarColor }]}>
                    {initials}
                  </Text>
                </View>
              )}
              <View style={s.modCam}>
                <CircleDashed size={12} color="white" />
              </View>
            </Hoverable>
            {uploading && (
              <ActivityIndicator
          ...
          {editing ? (
            <View style={s.modForm}>
              ...
              <Hoverable style={s.modSave} onPress={saveProfile}>
                <Text style={s.modSaveText}>Update Profile</Text>
              </Hoverable>
            </View>
          ) : (
            <View style={s.modInfo}>
              <Text style={s.modNameText}>{profile.name}</Text>
              <Text style={s.modPhoneText}>
          ...
          <View style={s.modSection}>
            ...
            <View style={s.modCard}>
              <View style={s.modRow}>
                ...
                <Text style={s.modRowLabel}>Shake to SOS</Text>
                <Switch
                  value={true}
              ...
              <View style={s.modRow}>
                ...
                <Text style={s.modRowLabel}>Alerts</Text>
                <Switch
                  value={true}
                  trackColor={{ true: COLORS.PRIMARY + "80" }}
                  thumbColor={COLORS.PRIMARY}
              ...
              <Hoverable
                style={[s.modRow, { borderBottomWidth: 0 }]}
                onPress={onSignOut}
              >
                <LogOut size={18} color="#ef4444" />
                <Text style={[s.modRowLabel, { color: "#ef4444" }]}>
                  Sign Out
                </Text>
              </Hoverable>
            </View>
