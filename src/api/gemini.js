...
export async function safetyChat(history, query, context = {}) {
  const { location = "Unknown", time = new Date().toLocaleTimeString() } =
    context;
...
export async function analyzeEvidence(base64Data, mimeType = "image/jpeg") {
  ...
  const payload = {
    systemInstruction: {
      parts: [
        {
          text: "You are an expert forensic evidence analyzer for the ShieldHer women's safety app. Your goal is to analyze the provided image (screenshot, photo, or document) and extract factual incident details. Return ONLY a valid JSON object.",
        },
      ],
    },
    contents: [
...
Return ONLY a valid JSON object:
{
  ...
  const messages = [
    {
      role: "user",
      content: `Context: Time: ${time}, City: ${city}, Reports: ${JSON.stringify(reports)}`,
    },
  ];
...
{ "patternFound": boolean, "summary": "<1-2 sentences>", "recommendation": "<action step>", "urgency": "low" | "medium" | "high" }`;
  const messages = [
    { role: "user", content: `Incidents: ${JSON.stringify(incidents)}` },
  ];
