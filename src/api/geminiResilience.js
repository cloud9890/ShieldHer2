...
async function attemptGemini(key, model, system, messages, maxTokens) {
  ...
  if (!res.ok) {
    const err = new Error(
      data.error?.message || "Gemini AI generation failed.",
    );
  ...
  for (const model of models) {
    for (const key of UNIQUE_KEYS) {
      ...
      for (let attempt = 0; attempt < 3; attempt++) {
        try {
          const response = await attemptGemini(
            key,
            model,
            system,
            messages,
          ...
          if (err.status === 429) {
            console.warn(
              `[Gemini 429] Rate limit hit on key ending in ...${key.slice(-4)}. Trying next key.`,
            );
            break;
          }
          ...
          if (err.status >= 500) {
            console.warn(
              `[Gemini 5xx] Server error on attempt ${attempt + 1}. Retrying...`,
            );
